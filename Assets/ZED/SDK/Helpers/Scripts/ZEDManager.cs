...
#pragma warning restore 414
    ...
    private void CheckStereoMode()
    {
        ...
        if (camLeftTransform && camRightTransform && cameraLeft.stereoTargetEye == StereoTargetEyeMask.Left) //We found both a left- and right-eye camera.
        {
            ...
            if (ZEDSupportFunctions.hasXRDevice() && allowARPassThrough) // DAMITH'S COMMENT: This if-statement turns on/off XR tracking system with ZED -> if (ZEDSupportFunctions.hasXRDevice() && allowARPassThrough)
            {
                isStereoRig = true;
